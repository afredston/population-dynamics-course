...
## Maximum sustainable yield in a logistic model  
...
This point is *an optimal harvest level from an economic standpoint* because it maximizes revenue by balancing production (of the fish stock) with returns (of harvest). There is no mathematical solution that yields greater yearly harvest. However, **it is not stable**, and is extremely sensitive to any level of overharvesting. Roughgarden and Smith (1996) wrote, "Maintaining the stock at the optimal size... is like balancing a marble on top of a dome." 

### Alternatives to MSY management

Are there more robust ways to achieve sustainable yields than setting harvest quotas equal to the maximum $dN/dt$? An alternative is a constant effort model, where $H$ is proportional to population size and **effort** (the intensity of fishing, e.g., hours on the water, number of nets deployed, etc.)
